...
export function LoginForm({ onSuccess, onError, onToggleMode }: LoginFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (errorCode === 'auth/unauthorized-domain' || err.message.includes('domínio não autorizado')) {
        ...
        onError(err.message.includes('auth/invalid-credential') || err.message.includes('auth/wrong-password') ? 'Email ou senha incorretos.' : `Erro de Login: ${err.message}`);
      }
    } finally {
      setLoading(false);
    }
